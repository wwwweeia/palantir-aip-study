...
```mermaid
flowchart TD
    AIP["AIP 应用层<br/>AIP Logic · Workshop · Assist · Agent Studio · Threads<br/>LLM 编排 · Agent · 对话式产品"]
    ONT["Ontology 语义 / 逻辑 / 执行层<br/>Object Types · Link Types · Action Types<br/>Functions · Interfaces · Roles<br/>企业的「数字孪生」"]
    FDY["Foundry 数据平台层<br/>Pipeline Builder · Code Repo · Dataset · Compute<br/>ETL · 数据治理 · 计算引擎"]

    AIP -->|读写| ONT
    ONT -->|映射| FDY

    style AIP fill:#ede7f6,stroke:#7b1fa2
    style ONT fill:#e3f2fd,stroke:#1565c0
    style FDY fill:#e8f5e9,stroke:#2e7d32
```
...
flowchart LR
    ...
    P["Product<br/>涡轮叶片"]
    F["Factory<br/>汉堡工厂"]
    D["Delivery DHL-88492<br/>status: 质检中<br/>ETA: 下周二"]
...
classDiagram
    ...
    Ticket <|.. Bug : implements
    Ticket <|.. FeatureRequest : implements
```
...
```mermaid
flowchart TD
    APP["应用与集成层<br/>AIP Chatbot · Workshop · AIP Logic · Agent · API"]

    subgraph ONT["Palantir Ontology"]
        direction TB
        O1["语义层：Object Types · Properties · Link Types · Interfaces"]
        O2["逻辑层：Functions（TypeScript / Python）"]
        O3["执行层：Action Types（权限 · 校验 · 副作用 · 审计）"]
        O4["权限层：Roles（从数据到 AI 的统一权限控制）"]
        O1 --> O2 --> O3 --> O4
    end

    FDY["Palantir Foundry<br/>数据接入 · Pipeline Builder · 数据集管理"]
    SRC["数据源<br/>ERP · CRM · WMS · MES · IoT · 外部 API · 文件"]

    APP -->|读写| O1
    O4 -->|映射| FDY
    FDY --> SRC

    style APP fill:#ede7f6,stroke:#7b1fa2
    style FDY fill:#e8f5e9,stroke:#2e7d32
