...
> 版本：D3.1（2026-05-25 更新：路径 B 工具链补充 UModel）
...
|----|------|--------|----------------------|
| 语义层 | **UModel**（推荐试验）/ DataHub / dbt Semantic Layer | 定义业务对象及其关系，暴露为 AI 可查询的对象图 | Ontology 的 Object + Link |
...
> **2026-05-25 更新**：阿里巴巴开源了 [UModel](https://github.com/alibaba/UnifiedModel)（Apache-2.0），是目前国产开源里概念对齐度最高的语义运行时，原生支持 MCP，Agent 可直接通过 `umodel-mcp` 连接。它同时覆盖"语义层 + Agent 接入"两层，比 DataHub + LangChain 分开拼的集成成本更低。项目仍处于早期（19 天），API 不稳定，**适合试验，暂不建议生产依赖**。
